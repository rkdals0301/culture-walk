...
const AboutPage = () => {
  return (
    ...
    >
      <section
        className='grid gap-4 border-y border-[var(--color-border-primary)] py-5 sm:flex sm:items-center sm:justify-between sm:gap-8'
        aria-labelledby='about-categories-title'
      >
        <div>
          <p className='text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-brand-primary)]'>탐색 범위</p>
      ...
      <section aria-labelledby='about-principles-title'>
        <div className='mb-3 flex items-end justify-between gap-4'>
          <div>
            <p className='text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-brand-primary)]'>서비스 원칙</p>
            <h2 id='about-principles-title' className='mt-1 text-2xl font-semibold tracking-[-0.03em]'>
              찾는 순간부터 방문까지
            </h2>
          </div>
          <Route aria-hidden='true' className='hidden size-7 text-[var(--color-accent-primary)] sm:block' strokeWidth={1.6} />
        </div>
      ...
      <aside className='grid gap-3 border-l-2 border-[var(--color-accent-primary)] pl-4 sm:pl-5'>
        <p className='text-xs font-bold uppercase tracking-[0.12em] text-[var(--color-accent-text)]'>제공 정보</p>
