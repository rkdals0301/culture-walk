...
const MapStatus = ({ kind, code, message, onRetry, onContinueWithList }: MapStatusProps) => {
  ...
  return (
    <div
      className='status-callout status-callout-shell flex size-full items-center justify-center p-4 sm:p-6'
      data-status={kind}
      role='alert'
    >
      <div className='status-callout-card w-full max-w-xl rounded-[1.25rem] p-5 text-[var(--color-text-primary)] sm:p-7'>
        <div className='status-callout-icon' aria-hidden='true'>
          <AlertCircle className='size-6' strokeWidth={2} />
        </div>
        <div className='min-w-0'>
          <h2 className='mt-4 text-xl font-bold tracking-tight sm:text-2xl'>{copy.title}</h2>
            ...
            <div className='mt-6 flex flex-wrap gap-2'>
              {onRetry && copy.retryLabel && (
                <button
                  type='button'
                  onClick={onRetry}
                  className='inline-flex min-h-11 items-center rounded-xl bg-[var(--color-brand-primary)] px-4 text-sm font-semibold text-[var(--color-brand-on-primary)] transition hover:bg-[var(--color-brand-hover)] active:bg-[var(--color-brand-active)]'
                >
                  {copy.retryLabel}
                </button>
              )}
              {onContinueWithList && copy.continueLabel && (
                <button
                  ...
                  className='soft-chip inline-flex min-h-11 items-center rounded-xl px-4 text-sm font-semibold transition hover:bg-[var(--color-interactive-hover)] active:bg-[var(--color-interactive-active)]'
                >
                  {copy.continueLabel}
                </button>
              )}
