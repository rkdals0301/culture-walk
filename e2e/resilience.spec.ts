import { expect, gotoApp, test, waitForAppHydration } from './support/test';
...
test('검색 결과가 없을 때 전체 필터 초기화로 기본 목록을 복구한다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '“존재하지않는행사-e2e” 검색 결과가 없습니다' })).toBeVisible();
  await page.getByRole('button', { name: '검색어만 지우기' }).click();
  await expect(page.getByLabel('문화행사 검색')).toHaveValue('');
  await expect(page.getByRole('link', { name: /문화산책 테스트 공연 2099-/ })).toBeVisible();

  await page.getByLabel('문화행사 검색').fill('존재하지않는행사-e2e');
  await page.getByRole('button', { name: '무료만' }).click();
  await expect(page.getByRole('button', { name: '모든 필터 초기화' })).toBeVisible();
...
test('피드 API 응답 오류 후 다시 시도로 정상 상태를 복구한다', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '“API-복구-e2e” 검색 결과가 없습니다' })).toBeVisible();
